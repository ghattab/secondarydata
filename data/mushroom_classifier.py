...
mode_dict = { 'nb': 'Gaussian Naive Bayes', 'log_reg': 'Logistic regression',
             'lda': 'Linear Discriminant Analysis', 'rf': 'Random forest classifier',
...
def get_model(mode):
    ...
    'qda':  Quadratic discriminant analysis, 'rf': Random forest classifier, 'mlp': 'Multi-layer Perceptron classifier'
...
def train_model(X_train, y_train, mode):
    ...
    mode: str,
    used classifier, 'nb': Naive Bayes, 'log_reg': Logistic regression, 'lda': Linear discriminant analysis,
    'qda':  Quadratic discriminant analysis, 'rfc': Random forest classifier, 'mlp': 'Multi-layer Perceptron classifier'
...
def cross_fold_validation(data, **kwargs):
    ...
    score method, corresponds to scoring in sklearn.model_selection.cross_val_score (look there for details)
    mode: str, default='log_reg',
    used classifier, 'nb': Naive Bayes, 'log_reg': Logistic regression, 'lda': Linear discriminant analysis,
    'qda':  Quadratic discriminant analysis, 'rfc': Random forest classifier
...
if __name__ == "__main__":
    ...
    data_secondary = pd.read_csv(dataset_categories.FILE_PATH_SECONDARY_NO_MISS, sep=';', header=0, low_memory=False)
    ...
    data_new_matched = pd.read_csv(dataset_categories.FILE_PATH_SECONDARY_MATCHED, header=0, sep=';')
    data_1987_matched = pd.read_csv(dataset_categories.FILE_PATH_1987_MATCHED, header=0, sep=';')
